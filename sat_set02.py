...
class SatSetting(object):
    ...
    def data_bound(self):
        ...
        if self.temp_n < 18.3:
            temp_min = 12.7
            temp_max = 18.3
        if (self.temp_n >= 18.3) & (self.temp_n <= 21.1):
            temp_min = -0.39 * self.temp_n +19.8
            temp_max = -2.4 * self.temp_n + 62.2

        if self.sp_sat_alt > temp_max:
            self.sp_sat_alt = temp_max
        if self.sp_sat_alt < temp_min:
            self.sp_sat_alt = temp_min
